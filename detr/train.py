...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='detr/output',
                        help='path where to save, empty for no saving')
    parser.add_argument('--device', default='cuda',
                        help='device to use for training / testing')
    parser.add_argument('--seed', default=42, type=int)
    parser.add_argument('--resume', default='detr/detr-r50-e632da11.pth', help='resume from checkpoint')
...
def main(args):
    ...
    if args.eval:
        ...
        for i in range(dataset_num):
            ...
            model_eval = mlflow.pytorch.load_model(model_uri)
            test_stats, coco_evaluator = evaluate(model_eval, criterion, postprocessors,
                                                data_loader_val, base_ds, device, args.output_dir)
            
            mlflow.end_run()
            eval_experiment_name ="DETR evals"
            ...
            mlflow.log_metric("mAP 0.50_0.95", test_stats['coco_eval_bbox'][0])
            if args.output_dir:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        mlflow.log_metric("val_loss", test_stats["loss"], step=epoch)
        mlflow.log_metric("val_accuracy", coco_evaluator.coco_eval['bbox'].stats.tolist()[0], step=epoch)
